...
namespace Burgerama.Services.Ratings.Endpoint.Handlers
{
    public sealed class OutingCreatedHandler : Consumes<OutingCreated>.Context
    {
        ...
        public OutingCreatedHandler(ILogger logger, ICandidateRepository candidateRepository)
        {
            _logger = logger;
        ...
        {
            var venue = _candidateRepository.Get(context.Message.VenueId);
            if (venue == null)
            {
                ...
                venue = new Candidate(context.Message.VenueId, string.Empty);
